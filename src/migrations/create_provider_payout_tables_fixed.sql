...
CREATE INDEX IF NOT EXISTS idx_provider_payout_methods_provider ON provider_payout_methods(provider_id);
CREATE INDEX IF NOT EXISTS idx_provider_payout_methods_default ON provider_payout_methods(provider_id, is_default) WHERE is_default = true;
...
CREATE INDEX IF NOT EXISTS idx_provider_payouts_scheduled ON provider_payouts(scheduled_date) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_provider_revenue_provider ON provider_revenue(provider_id);
CREATE INDEX IF NOT EXISTS idx_provider_revenue_booking ON provider_revenue(booking_id);
CREATE INDEX IF NOT EXISTS idx_provider_revenue_status ON provider_revenue(status);
...
DROP TRIGGER IF EXISTS update_provider_payout_methods_updated_at ON provider_payout_methods;
...
DROP POLICY IF EXISTS "Providers can view own payout methods" ON provider_payout_methods;
DROP POLICY IF EXISTS "Providers can insert own payout methods" ON provider_payout_methods;
DROP POLICY IF EXISTS "Providers can update own payout methods" ON provider_payout_methods;
DROP POLICY IF EXISTS "Providers can view own payouts" ON provider_payouts;
DROP POLICY IF EXISTS "Providers can view own revenue" ON provider_revenue;
...
-- Create or replace view for provider financial summary
