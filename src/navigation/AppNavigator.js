...
const useAuth = () => {
  console.log(`[DEBUG][${new Date().toISOString()}] useAuth - Hook starting`);
  ...
  console.log(`[DEBUG][${new Date().toISOString()}] useAuth - Returning:`, { isAuthenticated, isLoading, userRole });
...
function AppNavigator() {
  ...
  console.log(`[DEBUG][${new Date().toISOString()}] AppNavigator - About to call useAuth`);
  let authData;
  try {
    authData = useAuth();
    console.log(`[DEBUG][${new Date().toISOString()}] AppNavigator - useAuth returned:`, authData);
  } catch (error) {
    console.error(`[DEBUG][${new Date().toISOString()}] AppNavigator - useAuth ERROR:`, error);
    return <Text>Error in useAuth hook</Text>;
  }
  const { isAuthenticated, isLoading, userRole } = authData;
  ...
  try {
    ...
    console.error(`[DEBUG][${new Date().toISOString()}] AppNavigator - Error rendering NavigationContainer:`, error);
    return <Text>Error loading navigation</Text>;
  }
}
