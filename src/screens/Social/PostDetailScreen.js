...
import { getEnhancedImageProps, isValidImageUrl } from '../../utils/imageErrorHandler';
...
const PostDetailScreen = ({ route, navigation }) => {
  ...
  const handleImageError = (error, uri, imageType) => {
    if (__DEV__) {
      console.warn('PostDetailScreen: Image failed to load', {
        imageType,
    ...
    >
      ...
      <ScrollView style={styles.scrollView}>
        <View style={styles.postContainer}>
          <View style={styles.postHeader}>
            <Image 
              {...getEnhancedImageProps(
                posterProfile?.avatar_url || 'https://smtckdlpdfvdycocwoip.supabase.co/storage/v1/object/public/avatars/default-avatar.png',
                'post_detail_avatar',
          ...
          {post.media_urls && post.media_urls.length > 0 && isValidImageUrl(post.media_urls[0]) && (
            <Image 
              {...getEnhancedImageProps(
                post.media_urls[0],
                'post_detail_media',
