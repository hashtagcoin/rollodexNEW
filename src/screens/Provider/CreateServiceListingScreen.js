...
const CustomDropdown = ({ options, selectedValue, onSelect, placeholder }) => {
  ...
  return (
    <View style={[styles.dropdownContainer, isOpen && { zIndex: 1000 }]}>
      ...
      {isOpen && (
        <View style={[styles.dropdownMenu, { zIndex: 1001 }]}>
...
const CreateServiceListingScreen = ({ navigation }) => {
  ...
  const handleImageError = (error, uri, imageType) => {
    if (__DEV__) {
      console.warn('CreateServiceListingScreen: Image failed to load', {
        imageType,
              ...
              {images.map((image, index) => (
                <View key={index} style={styles.imageContainer}>
                  <Image 
                    ...
                    onLoadStart={() => {
                      if (__DEV__) {
                        console.log('CreateServiceListingScreen: Image preview loading started', { imageIndex: index });
                      }
                    }}
                    onPartialLoad={() => {
                      if (__DEV__) {
                        console.log('CreateServiceListingScreen: Image preview partial load', { imageIndex: index });
                      }
                    }}
