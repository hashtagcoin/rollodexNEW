...
const UserProfileScreen = () => {
  ...
  const handleImageError = (error, uri, imageType) => {
    if (__DEV__) {
      console.warn('UserProfileScreen: Image failed to load', {
        imageType,
          ...
          <View style={styles.postItemContainer}>
            {item.image_url && 
              <Image 
                ...
                style={styles.postImage_small} 
                resizeMode="cover"
                onError={(error) => handleImageError(error, item.image_url, 'post')}
                onLoadStart={() => {
                  if (__DEV__) {
                    console.log('UserProfileScreen: Post image loading started', { postId: item.id });
                  }
                }}
                onPartialLoad={() => {
                  if (__DEV__) {
                    console.log('UserProfileScreen: Post image partial load', { postId: item.id });
                  }
                }}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.friendItemContainer} onPress={() => navigation.push('UserProfileScreen', { userId: item.userId })}>
            <Image 
              ...
              onError={(error) => handleImageError(error, item.avatar, 'friend_avatar')}
              onLoadStart={() => {
                if (__DEV__) {
                  console.log('UserProfileScreen: Friend avatar loading started', { userId: item.userId });
                }
              }}
              onPartialLoad={() => {
                if (__DEV__) {
                  console.log('UserProfileScreen: Friend avatar partial load', { userId: item.userId });
                }
              }}
              defaultSource={require('../../../assets/placeholder-image.png')}
              loadingIndicatorSource={require('../../../assets/placeholder-image.png')}
        ...
        <View style={styles.postItemContainer}>
          {item.image_url && 
            <Image 
              ...
              style={styles.postImage_small} 
              resizeMode="cover"
              onError={(error) => handleImageError(error, item.image_url, 'post_grid')}
              onLoadStart={() => {
                if (__DEV__) {
                  console.log('UserProfileScreen: Post grid image loading started', { postId: item.id });
                }
              }}
              onPartialLoad={() => {
                if (__DEV__) {
                  console.log('UserProfileScreen: Post grid image partial load', { postId: item.id });
                }
              }}
              defaultSource={require('../../../assets/placeholder-image.png')}
              loadingIndicatorSource={require('../../../assets/placeholder-image.png')}
