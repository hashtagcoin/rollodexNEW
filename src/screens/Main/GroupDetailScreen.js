...
const GroupDetailScreen = () => {
  ...
  const handleImageError = (error, uri, imageType) => {
    if (__DEV__) {
      console.warn('GroupDetailScreen: Image failed to load', {
        imageType,
          ...
          <View key={item.id} style={styles.postCard}>
            ...
            {item.media_url && 
              <Image 
                ...
                resizeMode="cover"
                onError={(error) => handleImageError(error, item.media_url, 'post_media')}
                onLoadStart={() => {
                  if (__DEV__) {
                    console.log('GroupDetailScreen: Post media loading started', { postId: item.id });
                  }
                }}
                onPartialLoad={() => {
                  if (__DEV__) {
                    console.log('GroupDetailScreen: Post media partial load', { postId: item.id });
                  }
                }}
        ...
        {members.map(item => (
          <View key={item.id} style={styles.memberItem}>
            <Image 
              ...
              onError={(error) => handleImageError(error, item.avatarUrl, 'member_avatar')}
              onLoadStart={() => {
                if (__DEV__) {
                  console.log('GroupDetailScreen: Member avatar loading started', { memberId: item.id });
                }
              }}
              onPartialLoad={() => {
                if (__DEV__) {
                  console.log('GroupDetailScreen: Member avatar partial load', { memberId: item.id });
                }
              }}
              defaultSource={require('../../../assets/placeholder-image.png')}
              loadingIndicatorSource={require('../../../assets/placeholder-image.png')}
        ...
        {group && (
          <>
            <View style={styles.heroContainer}>
              <Image 
                source={{ uri: group.imageurl || 'https://via.placeholder.com/400x180.png?text=Group+Cover' }} 
                style={styles.coverImage}
                onError={(error) => handleImageError(error, group.imageurl, 'cover_image')}
                onLoadStart={() => {
                  if (__DEV__) {
                    console.log('GroupDetailScreen: Cover image loading started', { groupId: group.id });
                  }
                }}
                onPartialLoad={() => {
                  if (__DEV__) {
                    console.log('GroupDetailScreen: Cover image partial load', { groupId: group.id });
                  }
                }}
                defaultSource={require('../../../assets/placeholder-image.png')}
                loadingIndicatorSource={require('../../../assets/placeholder-image.png')}
