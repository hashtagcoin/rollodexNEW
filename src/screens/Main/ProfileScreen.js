...
// Default placeholder image as a data URI to avoid DNS resolution issues
const DEFAULT_AVATAR_URI = 'data:image/svg+xml;base64,';
const DEFAULT_IMAGE_URI = 'data:image/svg+xml;base64,';
...
const ProfileScreen = () => {
  ...
  const handleImageError = (error, uri, imageType) => {
    if (__DEV__) {
      console.warn('ProfileScreen: Image failed to load', {
        imageType,
  ...
  const fetchFavoriteGroups = async () => {
    ...
    try {
      // Fetch favorites from the favorites table - groups and housing groups
      ...
      // The favorites table already contains the item details (item_title, item_description, etc.)
      // so we can use them directly without additional queries
  ...
  const fetchFriendRequests = async () => {
    ...
    try {
      ...
      const formattedIncoming = incomingData.map(request => ({
        ...
        avatar: request.requester_avatar || DEFAULT_AVATAR_URI,
      ...
      const formattedOutgoing = outgoingData.map(request => ({
        ...
        avatar: request.addressee_avatar || DEFAULT_AVATAR_URI,
        timestamp: request.created_at,
        type: 'outgoing'
      }));
    ...
    try {
      ...
      const formattedFriends = data.map(friendship => {
        ...
        return {
          ...
          avatar: isRequester ? (friendship.addressee_avatar || DEFAULT_AVATAR_URI) : (friendship.requester_avatar || DEFAULT_AVATAR_URI),
  ...
  const startChat = async (userId) => {
    ...
    if (user) {
      ...
      setSelectedChatUser({
        ...
        avatar: user.avatar_url || DEFAULT_AVATAR_URI,
                  ...
                  return (
                    ...
                    >
                      <Image 
                        ...
                        onError={(error) => handleImageError(error, imageUrl, 'post')}
                        onLoadStart={() => {
                          if (__DEV__) {
                            console.log('ProfileScreen: Post image loading started', { postId: item.post_id });
                          }
                        }}
                        onPartialLoad={() => {
                          if (__DEV__) {
                            console.log('ProfileScreen: Post image partial load', { postId: item.post_id });
                          }
                        }}
            ...
            {loadingGroups ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    <Image 
                      source={{ uri: item.item_image_url || DEFAULT_IMAGE_URI }} 
                    ...
                    <>
                      <View style={styles.modalUserHeader}>
                        <Image 
                          ...
                          onError={(error) => handleImageError(error, selectedFriend.avatar, 'modal_avatar')}
                          onLoadStart={() => {
                            if (__DEV__) {
                              console.log('ProfileScreen: Modal avatar loading started', { friendId: selectedFriend.id });
                            }
                          }}
                          onPartialLoad={() => {
                            if (__DEV__) {
                              console.log('ProfileScreen: Modal avatar partial load', { friendId: selectedFriend.id });
                            }
                          }}
                          defaultSource={require('../../../assets/placeholder-image.png')}
                          loadingIndicatorSource={require('../../../assets/placeholder-image.png')}
                      ...
                      renderItem={({ item }) => {
                        ...
                        return (
                          <View style={styles.userItem}>
                            <Image 
                              source={{ 
                                uri: item.avatar_url || DEFAULT_AVATAR_URI
                              }} 
      ...
      >
        ...
        <View style={styles.profileTopSection}>
          ...
          <View style={styles.profileInfo}>
            ...
            <Text style={styles.ndisParticipant}>
              {userProfile?.role === 'provider' ? 'NDIS Provider' : 'NDIS Participant'}
            </Text>
          </View>
        ...
        <View style={styles.profileButtonRow}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              if (userProfile?.role === 'provider') {
                navigation.navigate('NDISProviderDashboard');
              } else {
                navigation.navigate('NDISScreen');
              }
            }}
          >
