...
const HeaderComponent = memo(({ navigation }) => (
  <View style={styles.stickyHeader}>
    ...
    <TouchableOpacity style={styles.stickerBtn} onPress={() => navigation.navigate('Events')}>
...
const SocialFeedScreen = () => {
  ...
  const fetchPosts = useCallback(async (isRefreshing = false, isBackgroundRefresh = false) => {
    ...
    try {
      ...
      // Simplified query first to debug the issue
      ...
      if (error) {
        ...
        console.error('[SocialFeed] Error details:', JSON.stringify(error, null, 2));
        console.log('[SocialFeed] Platform:', Platform.OS);
      ...
      const newPosts = data || [];
      console.log('[SocialFeed] Fetched posts count:', newPosts.length);
      console.log('[SocialFeed] Platform:', Platform.OS);
      console.log('[SocialFeed] First post sample:', newPosts[0] ? JSON.stringify(newPosts[0], null, 2) : 'No posts');
      ...
      if (isMounted.current) {
        console.log('[SocialFeed] Finally block - resetting loading states');
        updateUIState({
          loading: false,
          refreshing: false
        });
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (cacheRef.current && cacheIsFresh && !comingFromBookmarks) {
        ...
        // Force clear cache when coming from bookmarks
        if (comingFromBookmarks) {
          cacheRef.current = null;
          lastFetchTime.current = 0;
        }
        // Fetch immediately if no fresh cache
        if (Platform.OS === 'web') {
          // On web, run immediately with shorter timeout
          const fetchTimeout = setTimeout(() => {
            if (loading && isMounted.current) {
              console.log('[SocialFeed] Web timeout - resetting loading state');
              updateUIState({ loading: false });
              setPosts([]);
            }
          ...
          // On mobile, use InteractionManager
          InteractionManager.runAfterInteractions(() => {
            // Add timeout to prevent infinite loading
            const fetchTimeout = setTimeout(() => {
              if (loading && isMounted.current) {
                console.log('[SocialFeed] Mobile timeout - resetting loading state');
                updateUIState({ loading: false });
                setPosts([]);
              }
    ...
    }, [loading, fetchPosts, updateUIState, navigation])
  );
