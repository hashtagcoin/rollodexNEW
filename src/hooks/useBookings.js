...
export const useBookings = (userId) => {
   ...
   */
  const createBooking = useCallback(async (serviceId, timeSlot, date, paymentData = {}) => {
    ...
    try {
      ...
      const { data: bookingData, error: bookingError } = await supabase
        ...
        .insert({
          ...
          status: 'pending',
          total_price: paymentData.totalCost || 0,
      ...
      // Create transaction record if payment data is provided
      if (paymentData && (paymentData.totalCost > 0 || paymentData.gapPayment > 0)) {
        // Determine transaction type based on payment structure
        let transactionType = 'manual_payment';
        
        if (paymentData.ndisCoverage > 0 && paymentData.gapPayment === 0) {
          transactionType = 'fully_funded';
        } else if (paymentData.ndisCoverage > 0 && paymentData.gapPayment > 0) {
        ...
        const { error: transactionError } = await supabase
          ...
          .insert({
            user_id: userId,
            type: 'purchase',
            amount: paymentData.totalCost || 0,
            description: `Service booking: ${paymentData.serviceName || 'Service'}`,
            transaction_type: transactionType,
            // booking_id: booking.id, // Temporarily comment out to avoid FK issues
            created_at: new Date().toISOString()
          });
          
        if (transactionError) {
          console.error('Error creating transaction record:', transactionError);
          // Don't fail the booking if transaction record fails
        }
        
        // Update user's wallet balance if NDIS coverage is used
        if (paymentData.ndisCoverage > 0) {
          try {
            ...
            if (!walletFetchError && walletData) {
              const newBalance = Math.max(0, walletData.total_balance - paymentData.ndisCoverage);
