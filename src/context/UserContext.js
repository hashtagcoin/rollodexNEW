...
export const UserProvider = ({ children }) => {
  ...
  const uploadAvatarDirectly = async (uri) => {
    ...
    try {
      ...
      let base64Data;
      let contentType = fileExt === 'png' ? 'image/png' : 'image/jpeg';
      
      console.log('Direct upload - preparing file:', fileName);
      
      try {
        if (Platform.OS === 'web') {
          // On web, the URI is already a data URI with base64 data
          if (uri.startsWith('data:image/')) {
            ...
            if (base64Match) {
              ...
              console.log('Web - extracted base64 data, length:', base64Data.length);
            } else {
              throw new Error('Invalid data URI format');
            }
          } else {
            throw new Error('Web platform requires data URI format');
          }
        } else {
