...
import { View, TouchableOpacity, Image, StyleSheet, ActivityIndicator, Text, ScrollView, Platform } from 'react-native';
...
export default function ModernImagePicker({ 
  ...
  const handleImageError = (error, uri) => {
    if (__DEV__) {
      console.warn('ModernImagePicker: Image failed to load', {
      ...
      return (
        <View key={isExisting ? `existing-${index}-${uri.substring(0,20)}` : `new-${index}-${uri.substring(0,20)}`} style={styles.imageContainer}>
          <Image 
            ...
            onError={(error) => handleImageError(error, uri)}
            onLoadStart={() => {
              if (__DEV__) {
                console.log('ModernImagePicker: Gallery image loading started', { uri: uri?.substring(0, 30) + '...' });
              }
            }}
            onPartialLoad={() => {
              if (__DEV__) {
                console.log('ModernImagePicker: Gallery image partial load', { uri: uri?.substring(0, 30) + '...' });
              }
            }}
    ...
    >
      {avatar ? (
        <Image 
          ...
          onError={(error) => handleImageError(error, avatar)}
          onLoadStart={() => {
            if (__DEV__) {
              console.log('ModernImagePicker: Avatar loading started', { uri: avatar?.substring(0, 30) + '...' });
            }
          }}
          onPartialLoad={() => {
            if (__DEV__) {
              console.log('ModernImagePicker: Avatar partial load', { uri: avatar?.substring(0, 30) + '...' });
            }
          }}
          defaultSource={require('../../assets/placeholder-image.png')}
          loadingIndicatorSource={require('../../assets/placeholder-image.png')}
