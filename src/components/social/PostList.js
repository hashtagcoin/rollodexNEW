...
import { View, Text, Image, TouchableOpacity, StyleSheet, FlatList, RefreshControl, Platform } from 'react-native';
...
const PostList = ({ posts, currentUser, onRefresh, refreshing = false, groupId }) => {
  ...
  const handleImageError = (error, uri, imageType) => {
    if (__DEV__) {
      console.warn('PostList: Image failed to load', {
        imageType,
    ...
    <View style={styles.postContainer}>
      <View style={styles.postHeader}>
        <Image 
          ...
          style={styles.avatar}
          onError={(error) => handleImageError(error, post.user_avatar, 'avatar')}
          onLoadStart={() => {
            if (__DEV__) {
              console.log('PostList: Avatar loading started', { userId: post.user_id });
            }
          }}
          onPartialLoad={() => {
            if (__DEV__) {
              console.log('PostList: Avatar partial load', { userId: post.user_id });
            }
          }}
      ...
      {post.image && (
        <Image 
          ...
          resizeMode="cover"
          onError={(error) => handleImageError(error, post.image, 'post')}
          onLoadStart={() => {
            if (__DEV__) {
              console.log('PostList: Post image loading started', { postId: post.id });
            }
          }}
          onPartialLoad={() => {
            if (__DEV__) {
              console.log('PostList: Post image partial load', { postId: post.id });
            }
          }}
          defaultSource={require('../../../assets/placeholder-image.png')}
          loadingIndicatorSource={require('../../../assets/placeholder-image.png')}
